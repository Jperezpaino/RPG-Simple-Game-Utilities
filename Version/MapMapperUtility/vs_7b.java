...
public class MapMapperUtility {
    ...
    private static void run() {
        ...
        // Construir paleta de COLOR_COUNT colores dominantes del tileset
        ...
        for (int row = 0; row < mapRows; row++) {
            for (int col = 0; col < mapCols; col++) {
                ...
                if (match != null) {
                    ...
                    // Añadir a desconocidos solo si no hay ya uno igual
                    boolean alreadyPresent = unknownChunks.stream()
                            .anyMatch(uc -> compareTiles(uc, chunk));
                    if (!alreadyPresent) {
                        // Copiar el chunk para no depender del subimage original
                        BufferedImage copy = new BufferedImage(TILE_SIZE, TILE_SIZE, BufferedImage.TYPE_INT_ARGB);
                        copy.createGraphics().drawImage(chunk, 0, 0, null);
                        unknownChunks.add(copy);
                    }
                }
            }
        }
    ...
    private static void showUI(BufferedImage tilesetImage, List<TileEntry> tiles,
        ...
        for (int i = 0; i < unknownChunks.size(); i++) {
            ...
            // Menú contextual: comparar con tile del tileset
    ...
    /** Cuenta el número de píxeles distintos entre dos tiles. */
