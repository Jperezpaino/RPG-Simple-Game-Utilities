...
public class MapMapperUtility {
    ...
    private static void run() {
        ...
        showUI(tilesetImage, tiles, mapImage, analyzedImage, unknownChunks, duplicateIndices);
    }
    ...
    private static void showUI(BufferedImage tilesetImage, List<TileEntry> tiles,
        ...
        analyzedScroll.setBorder(BorderFactory.createTitledBorder(
                "Mapa analizado (magenta=tile desconocida) — clic derecho para guardar"));
        analyzedScroll.getVerticalScrollBar().setUnitIncrement(16);
        analyzedScroll.getHorizontalScrollBar().setUnitIncrement(16);

        // Menú contextual del mapa analizado: guardar imagen + guardar unknowns
        JPopupMenu analyzedMenu = new JPopupMenu();
        JMenuItem saveAnalyzedItem = new JMenuItem("Guardar imagen analizada...");
        saveAnalyzedItem.addActionListener(e -> saveAnalyzedImage(analyzedImage));
        JMenuItem saveUnknownItem = new JMenuItem("Guardar tiles desconocidas en 'unknown'...");
        saveUnknownItem.addActionListener(e -> saveUnknownTiles(unknownChunks, frame));
        analyzedMenu.add(saveAnalyzedItem);
        analyzedMenu.addSeparator();
        analyzedMenu.add(saveUnknownItem);
        analyzedLabel.setComponentPopupMenu(analyzedMenu);
