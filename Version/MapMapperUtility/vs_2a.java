 ...
 *
 * Paso 2: Extracción y visualización de tiles individuales del tileset.
 *
...
public class MapMapperUtility {
    ...
    private static void run() {
        ...
        showTilesetAndTiles(tilesetImage, tiles);
    }
     ...
     */
    private static void showTilesetAndTiles(BufferedImage tilesetImage, List<TileEntry> tiles) {
        JFrame frame = new JFrame("MapMapperUtility - Extracción de tiles: clear-tiles");
        ...
        tilesetLabel.setBorder(BorderFactory.createTitledBorder("Tileset original (x" + DISPLAY_SCALE + ")"));
        JScrollPane leftScroll = new JScrollPane(tilesetLabel);
        leftScroll.setPreferredSize(new Dimension(
                scaledTileset.getWidth() + 30,
                scaledTileset.getHeight() + 40));
        ...
        rightScroll.setPreferredSize(new Dimension(320, scaledTileset.getHeight() + 40));
        rightScroll.getVerticalScrollBar().setUnitIncrement(16);

        JLabel rightTitle = new JLabel(
                " Tiles con contenido (" + tiles.stream().filter(t -> !t.empty).count() + " tiles)");
