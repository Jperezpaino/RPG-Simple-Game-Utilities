...
public class MapMapperUtility {
    ...
    private static void run() {
        ...
        // Detectar tiles duplicadas en el tileset y avisar con ventana de alerta
        detectAndWarnDuplicateTiles(tiles);
     ...
     * Si encuentra duplicados, muestra una ventana de alerta con el listado.
     */
    private static void detectAndWarnDuplicateTiles(List<TileEntry> tiles) {
        ...
        for (int i = 0; i < nonEmpty.size(); i++) {
            for (int j = i + 1; j < nonEmpty.size(); j++) {
                if (compareTiles(quantized.get(i), quantized.get(j))) {
                    ...
                    String msg = String.format("Tile #%02d [col=%d, row=%d]  ==  Tile #%02d [col=%d, row=%d]",
                            a.index, a.col, a.row, b.index, b.col, b.row);
                    messages.add(msg);
                    System.out.println("⚠ Duplicado: " + msg);
                }
            }
        }

        if (!messages.isEmpty()) {
            System.out.println("⚠ TOTAL DUPLICADOS EN TILESET: " + messages.size());
            String text = "Se han encontrado " + messages.size() + " tile(s) duplicada(s) en el tileset:\n\n"
                    + String.join("\n", messages);
            JOptionPane.showMessageDialog(null, text,
                    "⚠ Tiles duplicadas en el tileset", JOptionPane.WARNING_MESSAGE);
        } else {
