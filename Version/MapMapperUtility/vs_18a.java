...
public class MapMapperUtility {
    ...
    private static class SelectableMapPanel extends JPanel {
        ...
        /** Export Manager: lista de tiles únicas + preview del mapa + botones Aceptar/Cancelar.
         *  Si alguna tile no tiene nombre guardado ofrece un campo de texto para nominarla.
         *  Al aceptar genera un ZIP con: map.png, map.dat, map.info y carpeta tiles/. */
        private static void showExportManagerDialog(JFrame owner, TileEntry[][] selGrid,
                                                    int tileX, int tileY) {
            int th = selGrid.length;
            int tw = th > 0 ? selGrid[0].length : 0;

            // --- Tiles únicas (por índice, en orden de aparición) ---
            java.util.LinkedHashMap<Integer, TileEntry> uniqueTiles = new java.util.LinkedHashMap<>();
            for (TileEntry[] row : selGrid)
                for (TileEntry te : row)
                    uniqueTiles.putIfAbsent(te.index, te);

            // --- Índice local secuencial: tile.index → local id 0,1,2,... ---
            java.util.Map<Integer, Integer> localIdx = new java.util.LinkedHashMap<>();
            int li = 0;
            for (int idx : uniqueTiles.keySet())
                localIdx.put(idx, li++);

            // --- Imagen del mapa sin escalar (TILE_SIZE por celda) ---
            ...
            // ----------------------------------------------------------------
            // Panel izquierdo: lista de tiles únicas
            // ----------------------------------------------------------------
            ...
            // Campos de texto para tiles sin nombre: índice → JTextField
            java.util.Map<Integer, JTextField> nameFields = new java.util.LinkedHashMap<>();

            final int PREV_SCALE = 4; // preview ×4 = 64 px
            for (TileEntry te : uniqueTiles.values()) {
                JPanel rowPanel = new JPanel(new java.awt.FlowLayout(java.awt.FlowLayout.LEFT, 6, 3));
                rowPanel.setBackground(new Color(30, 35, 50));
                rowPanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 72));

                // Etiqueta con el id local
                JLabel idxLbl = new JLabel(String.valueOf(localIdx.get(te.index)));
                idxLbl.setForeground(new Color(120, 160, 255));
                idxLbl.setFont(new Font(Font.MONOSPACED, Font.BOLD, 11));
                idxLbl.setPreferredSize(new Dimension(26, 20));

                // Preview ×4
                BufferedImage prev = scaleNearest(te.image, PREV_SCALE);
                JLabel prevLbl = new JLabel(new ImageIcon(prev));
                ...
                if (te.name != null) {
                    // Tile ya guardada: mostrar nombre en verde
                    ...
                    // Tile sin guardar: campo de texto obligatorio
                    ...
                    tf.setToolTipText("Nombre requerido para exportar esta tile (índice " + te.index + ")");
                    nameFields.put(te.index, tf);
            ...
            tileScroll.setPreferredSize(new Dimension(290, 420));
            ...
            // ----------------------------------------------------------------
            int dispScale = DISPLAY_SCALE * 2; // base ×4
            int maxPreviewPx = 480;
            while (dispScale > 1 &&
                   (mapImage.getWidth() * dispScale > maxPreviewPx ||
                    mapImage.getHeight() * dispScale > maxPreviewPx))
                dispScale--;

            BufferedImage mapPreview = scaleNearest(mapImage, dispScale);
            JLabel mapPreviewLbl = new JLabel(new ImageIcon(mapPreview));
            ...
            // ----------------------------------------------------------------
            // Split horizontal: tiles | mapa
            // ----------------------------------------------------------------
            JSplitPane centerSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, tileScroll, mapScroll);
            ...
            // ----------------------------------------------------------------
            JPanel btnPanel = new JPanel(new java.awt.FlowLayout(java.awt.FlowLayout.RIGHT, 10, 8));
            btnPanel.setBackground(new Color(25, 30, 45));
            ...
            btnAccept.addActionListener(ev -> {
                // 1. Validar que todas las tiles sin nombre tienen texto introducido
                java.util.List<String> errors = new java.util.ArrayList<>();
                for (java.util.Map.Entry<Integer, JTextField> entry : nameFields.entrySet())
                    if (entry.getValue().getText().trim().isEmpty())
                        errors.add("  • Tile índice " + entry.getKey() + " no tiene nombre asignado.");
                if (!errors.isEmpty()) {
                    JOptionPane.showMessageDialog(dlg,
                            "No se puede exportar: hay tiles sin nombre:\n\n"
                            + String.join("\n", errors),
                            "Nombres incompletos", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                // 2. Guardar tiles sin nombre (nombre introducido por el usuario)
                for (java.util.Map.Entry<Integer, JTextField> entry : nameFields.entrySet()) {
                    TileEntry te = uniqueTiles.get(entry.getKey());
                    String name = entry.getValue().getText().trim();
                    saveTileToFile(te.image, name);
                    te.name = name; // actualizar en memoria
                }

                // 3. Seleccionar destino del ZIP
                javax.swing.JFileChooser fc = new javax.swing.JFileChooser();
                fc.setDialogTitle("Guardar exportación ZIP");
                fc.setSelectedFile(new File(
                        String.format("map_%d_%d_%dx%d.zip", tileX, tileY, tw, th)));
                fc.setFileFilter(
                        new javax.swing.filechooser.FileNameExtensionFilter("ZIP files (*.zip)", "zip"));
                ...
                try (java.util.zip.ZipOutputStream zos = new java.util.zip.ZipOutputStream(
                    ...
                    // --- map.dat (cuadrícula de índices locales) ---
                    StringBuilder dat = new StringBuilder();
                    for (int r = 0; r < th; r++) {
                        for (int c = 0; c < tw; c++) {
                            if (c > 0) dat.append(' ');
                            dat.append(localIdx.get(selGrid[r][c].index));
                        }
                        dat.append('\n');
                    }
                    ...
                    // --- map.info (id,nombre,fichero) ---
                    StringBuilder info = new StringBuilder();
                    info.append("id,name,file\n");
                    for (TileEntry te : uniqueTiles.values()) {
                        String fname = te.name.trim()
                                .replaceAll("[^a-zA-Z0-9_\\-]", "_") + ".png";
                        info.append(localIdx.get(te.index)).append(',')
                ...
                JOptionPane.showMessageDialog(dlg,
                        ...
                        + "\n  tiles/    — " + uniqueTiles.size() + " tile(s) únicas",
            ...
            dlg.add(btnPanel, BorderLayout.SOUTH);
            dlg.setPreferredSize(new Dimension(740, 540));
            dlg.pack();
            dlg.setLocationRelativeTo(owner);
            dlg.setVisible(true);
        }
