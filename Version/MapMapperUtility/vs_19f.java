...
public class MapMapperUtility {
    ...
    private static class SelectableMapPanel extends JPanel {
        ...
        private static boolean showExportManagerDialog(JFrame owner, TileEntry[][] selGrid,
            ...
            // ── Campos persistentes por tile (sobreviven al rebuild) ─────────
            // id editable: arranca en 0,1,2,... pero el usuario lo puede cambiar
            java.util.IdentityHashMap<TileEntry, JTextField> idFields   = new java.util.IdentityHashMap<>();
            // nombre: solo para tiles sin guardar
            java.util.IdentityHashMap<TileEntry, JTextField> nameFields = new java.util.IdentityHashMap<>();
            for (int i = 0; i < orderedTiles.size(); i++) {
                TileEntry te = orderedTiles.get(i);
                JTextField idf = new JTextField(String.valueOf(i), 4);
                idf.setFont(new Font(Font.MONOSPACED, Font.BOLD, 12));
                idf.setBackground(new Color(40, 45, 65));
                idf.setForeground(new Color(120, 180, 255));
                idf.setCaretColor(Color.WHITE);
                idf.setHorizontalAlignment(JTextField.CENTER);
                idf.setBorder(BorderFactory.createCompoundBorder(
                        BorderFactory.createLineBorder(new Color(60, 90, 160), 1),
            ...
            // ── Panel de lista de tiles (se reconstruye al reordenar) ────────
            JPanel tileListPanel = new JPanel();
            tileListPanel.setLayout(new BoxLayout(tileListPanel, BoxLayout.Y_AXIS));
            tileListPanel.setBackground(new Color(30, 35, 50));
            tileListPanel.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));
            ...
            javax.swing.border.TitledBorder tilesBorder = BorderFactory.createTitledBorder(
                    BorderFactory.createLineBorder(new Color(80, 100, 160)),
                    "Tiles únicas — ID editable · ▲▼ reordenar");
            tilesBorder.setTitleColor(new Color(180, 200, 255));
            tilesBorder.setTitleFont(new Font(Font.SANS_SERIF, Font.BOLD, 12));
            tileScroll.setBorder(tilesBorder);
            tileScroll.getVerticalScrollBar().setUnitIncrement(16);

            // ── Rebuilder: reconstruye las filas de la lista ─────────────────
            final Runnable[] rebuilder = {null};
            rebuilder[0] = () -> {
                // Guardar posición del scroll antes de limpiar
                int scrollVal = tileScroll.getVerticalScrollBar().getValue();
                tileListPanel.removeAll();
                ...
                for (int i = 0; i < orderedTiles.size(); i++) {
                    ...
                    upBtn.setMargin(new java.awt.Insets(1, 4, 1, 4));
                    dnBtn.setMargin(new java.awt.Insets(1, 4, 1, 4));
                    upBtn.setToolTipText("Mover arriba");
                    dnBtn.setToolTipText("Mover abajo");
                    upBtn.setEnabled(fi > 0);
                    dnBtn.setEnabled(fi < orderedTiles.size() - 1);
                    upBtn.addActionListener(ev -> {
                        TileEntry tmp = orderedTiles.get(fi - 1);
                    ...
                    // ── Campo de ID ───────────────────────────────────────
                    JTextField idf = idFields.get(te);
                    idf.setPreferredSize(new Dimension(42, 24));
            ...
            rebuilder[0].run(); // primera construcción

            // ── Panel derecho: preview del mapa ──────────────────────────────
            int dispScale = DISPLAY_SCALE * 2;
            int maxPx = 480;
            while (dispScale > 1 &&
                   (mapImage.getWidth() * dispScale > maxPx ||
            ...
            btnAccept.addActionListener(ev -> {
                ...
                // ── 2. Validar IDs (enteros, sin duplicados) ──────────────
                java.util.Map<TileEntry, Integer> customIds = new java.util.IdentityHashMap<>();
                java.util.Set<Integer> usedIds = new java.util.HashSet<>();
                for (TileEntry te : orderedTiles) {
                    String raw = idFields.get(te).getText().trim();
                    int id;
                    try { id = Integer.parseInt(raw); }
                    catch (NumberFormatException ex) {
                        JOptionPane.showMessageDialog(dlg,
                                "El ID \"" + raw + "\" no es un número entero válido.",
                                "ID inválido", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                    if (!usedIds.add(id)) {
                        JOptionPane.showMessageDialog(dlg,
                                "El ID " + id + " está asignado a más de una tile.",
                ...
                // ── 3. Guardar tiles sin nombre ───────────────────────────
                for (TileEntry te : orderedTiles)
                    if (te.name == null) {
                        String name = nameFields.get(te).getText().trim();
                        saveTileToFile(te.image, name);
                        te.name = name;  // actualizar en memoria → próxima apertura ya tiene nombre
                    }

                // ── 4. Seleccionar destino ZIP ────────────────────────────
                javax.swing.JFileChooser fc = new javax.swing.JFileChooser();
                fc.setDialogTitle("Guardar exportación ZIP");
                fc.setSelectedFile(new File(
                        String.format("map_%d_%d_%dx%d.zip", tileX, tileY, tw, th)));
                ...
                try (java.util.zip.ZipOutputStream zos = new java.util.zip.ZipOutputStream(
                    ...
                    // map.dat — cuadrícula de IDs customizados
                    StringBuilder dat = new StringBuilder();
                    for (int r = 0; r < th; r++) {
                        for (int c = 0; c < tw; c++) {
                            if (c > 0) dat.append(' ');
                            dat.append(customIds.get(selGrid[r][c]));
                        }
                        dat.append('\n');
                    }
                    zos.putNextEntry(new java.util.zip.ZipEntry("map.dat"));
                    ...
                    for (TileEntry te : orderedTiles) {
                        String fname = te.name.trim()
                                .replaceAll("[^a-zA-Z0-9_\\-]", "_") + ".png";
                        info.append(customIds.get(te)).append(',')
                            .append(te.name).append(',')
                            .append("tiles/").append(fname).append('\n');
                    }
                    zos.putNextEntry(new java.util.zip.ZipEntry("map.info"));
            ...
            dlg.add(centerSplit, BorderLayout.CENTER);
            dlg.add(btnPanel, BorderLayout.SOUTH);
            dlg.setPreferredSize(new Dimension(790, 560));
            dlg.pack();
            dlg.setLocationRelativeTo(owner);
            dlg.setVisible(true);
            return exported[0];
