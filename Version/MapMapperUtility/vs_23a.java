...
public class MapMapperUtility {
    ...
    /** Fichero índice de tiles conocidas (fuente y compilado). */
    private static final String TILE_INFO_SRC    = "src/main/resources/assets/map/world/tile.info";
    private static final String TILE_INFO_BIN    = "target/classes/assets/map/world/tile.info";
    /** Directorio base para resolver las rutas relativas de tile.info. */
    private static final String TILE_BASE_SRC    = "src/main/resources/assets/map/world";
    private static final String TILE_BASE_BIN    = "target/classes/assets/map/world";
    ...
    /**
     * Carga todas las tiles registradas en tile.info.
     * Formato de cada línea (tras la cabecera): id,name,file
     * donde «file» es relativa al directorio base (assets/map/world/).
     */
    private static List<TileEntry> loadSavedTiles() {
        List<TileEntry> result = new ArrayList<>();
        File infoFile = new File(TILE_INFO_SRC);
        if (!infoFile.exists()) {
            System.err.println("tile.info no encontrado: " + infoFile.getAbsolutePath());
            return result;
        }
        int startIndex = 1000; // índices altos para no colisionar con el tileset
        try (java.io.BufferedReader br = new java.io.BufferedReader(
                new java.io.FileReader(infoFile, java.nio.charset.StandardCharsets.UTF_8))) {
            String line;
            boolean first = true;
            while ((line = br.readLine()) != null) {
                if (first) { first = false; continue; } // saltar cabecera id,name,file
                line = line.trim();
                if (line.isEmpty()) continue;
                String[] parts = line.split(",", 3);
                if (parts.length < 3) {
                    System.err.println("Línea mal formada en tile.info: " + line);
                    continue;
                }
                String name     = parts[1].trim();
                String filePath = parts[2].trim(); // ej. "tile/grass.png"
                File imgFile = new File(TILE_BASE_SRC, filePath);
                if (!imgFile.exists()) {
                    System.err.println("Tile no encontrada: " + imgFile.getAbsolutePath());
                    continue;
                }
                try {
                    ...
                    System.err.println("Error al cargar tile \"" + name + "\": " + e.getMessage());
                }
            }
        } catch (IOException e) {
            System.err.println("Error al leer tile.info: " + e.getMessage());
        }
        return result;
    }
     ...
     * y registra la nueva entrada en tile.info (ambos directorios).
    ...
    private static boolean saveTileToFile(BufferedImage img, String name) {
        ...
        // ── 2. Actualizar tile.info ───────────────────────────────────────
        String relPath = "tile/" + filename; // relativo al directorio base world/
        for (String infoPath : new String[]{ TILE_INFO_SRC, TILE_INFO_BIN }) {
            File infoFile = new File(infoPath);
            // Calcular el próximo ID leyendo el actual máximo
            int nextId = 0;
            if (infoFile.exists()) {
                try (java.io.BufferedReader br = new java.io.BufferedReader(
                        new java.io.FileReader(infoFile, java.nio.charset.StandardCharsets.UTF_8))) {
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        String[] parts = line.split(",", 3);
                        if (parts.length >= 1) {
                            try { nextId = Math.max(nextId, Integer.parseInt(parts[0].trim()) + 1); }
                            catch (NumberFormatException ignored) {}
                        }
                    }
                } catch (IOException e) {
                    System.err.println("Error al leer tile.info para calcular ID: " + e.getMessage());
                }
            }
            // Asegurar que existe el directorio destino
            if (infoFile.getParentFile() != null) infoFile.getParentFile().mkdirs();
            // Añadir cabecera si el fichero no existe o está vacío
            boolean needHeader = !infoFile.exists() || infoFile.length() == 0;
            try (java.io.PrintWriter pw = new java.io.PrintWriter(
                    new java.io.OutputStreamWriter(
                        new java.io.FileOutputStream(infoFile, true),
                        java.nio.charset.StandardCharsets.UTF_8))) {
                if (needHeader) pw.println("id,name,file");
                pw.println(nextId + "," + name.trim() + "," + relPath);
                System.out.println("tile.info actualizado (" + infoPath + "): "
                        + nextId + "," + name.trim() + "," + relPath);
            } catch (IOException e) {
                System.err.println("Error al actualizar tile.info en " + infoPath + ": " + e.getMessage());
                ok = false;
            }
        }
        return ok;
    ...
    private static void showUI(BufferedImage tilesetImage, List<TileEntry> tiles,
        ...
        // (constructed after refreshTilesList is defined — see below)
        ...
        // SelectableMapPanel needs refreshTilesList — constructed after it is defined (see below)
        ...
        // Rebuilder: se llama al inicio y cada vez que se guardan tiles nuevas
        Runnable refreshTilesList = () -> {
            ...
            for (TileEntry entry : tiles) {
                ...
                if (entry.name == null) {
                    ...
                    String dialogLabel = String.format("Tile #%02d  [col %d, fila %d]",
                            entry.index, entry.col, entry.row);
            ...
            long nonEmpty2 = tiles.stream().filter(t -> !t.empty).count();
            rightTitle.setText(" Tiles conocidas (" + nonEmpty2 + " / " + tiles.size() + ")");
            tilesPanel.revalidate();
            tilesPanel.repaint();
        };
        refreshTilesList.run();
    ...
    private static class SelectableMapPanel extends JPanel {
        ...
        private final Runnable onTilesUpdated;
        ...
        SelectableMapPanel(BufferedImage displayImage, int scale, JFrame owner,
                           ...
                           TileEntry[][] grid, Runnable onTilesUpdated) {
            ...
            this.grid            = grid;
            this.onTilesUpdated  = onTilesUpdated;
        ...
        private void handleRightClick(Point p) {
            if (sel != null && sel.contains(p)) {
                ...
                JMenuItem exportMapItem = new JMenuItem("Exportar Mapa...");
                exportMapItem.addActionListener(ev -> exportMap());
        ...
        /** Valida la selección y abre el gestor de exportación de mapa. */
        private void exportMap() {
            if (sel == null) return;
            int tx = sel.x / tilePx();
            int ty = sel.y / tilePx();
            int tw = sel.width / tilePx();
            int th = sel.height / tilePx();

            // Verificar que ninguna celda dentro de la selección sea desconocida
            java.util.List<String> missing = new java.util.ArrayList<>();
            for (int r = 0; r < th; r++) {
                for (int c = 0; c < tw; c++) {
                    int gr = ty + r, gc = tx + c;
                    if (gr >= grid.length || gc >= grid[gr].length || grid[gr][gc] == null) {
                        missing.add("  (" + gc + ", " + gr + ")");
                    }
                }
            }
            if (!missing.isEmpty()) {
                JOptionPane.showMessageDialog(owner,
                        "La selección contiene " + missing.size() + " celda(s) sin tile definida:\n\n"
                        + String.join("\n", missing),
                        "Error: tiles desconocidas en la selección",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }

            // Extraer subgrid de la selección
            TileEntry[][] selGrid = new TileEntry[th][tw];
            for (int r = 0; r < th; r++)
                for (int c = 0; c < tw; c++)
                    selGrid[r][c] = grid[ty + r][tx + c];

            boolean exported = showExportManagerDialog(owner, selGrid, tx, ty, onTilesUpdated);
            if (exported) { sel = null; repaint(); }
        }

        /** Export Manager: lista de tiles únicas del mapa seleccionado.
         *  Tiles ya guardadas → ID fijo de tile.info, botones ▲▼ desactivados.
         *  Tiles nuevas (sin nombre) → ID provisional (maxId+1, maxId+2, …),
         *  reordenables entre sí con ▲▼.
         *  Al aceptar: guarda nuevas tiles, genera ZIP con IDs reales. */
        private static boolean showExportManagerDialog(JFrame owner, TileEntry[][] selGrid,
                                                       int tileX, int tileY,
                                                       Runnable onTilesUpdated) {
            final int th = selGrid.length;
            final int tw = th > 0 ? selGrid[0].length : 0;

            // ── Lista ordenada de tiles únicas ───────────────────────────────
            java.util.List<TileEntry> orderedTiles = new java.util.ArrayList<>();
            java.util.Set<Integer> seen = new java.util.LinkedHashSet<>();
            for (TileEntry[] row : selGrid)
                for (TileEntry te : row)
                    if (seen.add(te.index)) orderedTiles.add(te);

            // ── Leer tile.info para obtener IDs fijos ────────────────────────
            // nameToFixedId: nombre → id (solo tiles ya registradas)
            java.util.Map<String, Integer> nameToFixedId = new java.util.LinkedHashMap<>();
            int maxRegisteredId = -1;
            File tileInfoFile = new File(TILE_INFO_SRC);
            if (tileInfoFile.exists()) {
                try (java.io.BufferedReader br = new java.io.BufferedReader(
                        new java.io.FileReader(tileInfoFile, java.nio.charset.StandardCharsets.UTF_8))) {
                    ...
                    while ((ln = br.readLine()) != null) {
                        ...
                        if (p.length >= 2) {
                            try {
                                ...
                                nameToFixedId.put(p[1].trim(), id);
                                if (id > maxRegisteredId) maxRegisteredId = id;
                            } catch (NumberFormatException ignored) {}
                        }
                    }
                } catch (IOException e) {
                    System.err.println("No se pudo leer tile.info: " + e.getMessage());
                }
            }
            // nextNewId: contador para asignar IDs a tiles nuevas (no registradas)
            final int[] nextNewId = { maxRegisteredId + 1 };

            // ── Campos de nombre — solo para tiles sin guardar ───────────────
            java.util.IdentityHashMap<TileEntry, JTextField> nameFields = new java.util.IdentityHashMap<>();
            for (TileEntry te : orderedTiles)
                if (te.name == null) {
                    JTextField tf = new JTextField(14);
                    tf.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 12));
                    tf.setBackground(new Color(50, 55, 75));
                    tf.setForeground(Color.WHITE);
                    tf.setCaretColor(Color.WHITE);
                    tf.setBorder(BorderFactory.createCompoundBorder(
                            BorderFactory.createLineBorder(new Color(220, 80, 60), 1),
                            BorderFactory.createEmptyBorder(2, 4, 2, 4)));
                    tf.setToolTipText("Nombre requerido para exportar");
                    nameFields.put(te, tf);
                }

            // ── Imagen del mapa (sin escalar) ────────────────────────────────
            BufferedImage mapImage = new BufferedImage(
                    tw * TILE_SIZE, th * TILE_SIZE, BufferedImage.TYPE_INT_ARGB);
            Graphics2D gm = mapImage.createGraphics();
            for (int r = 0; r < th; r++)
                for (int c = 0; c < tw; c++)
                    gm.drawImage(selGrid[r][c].image, c * TILE_SIZE, r * TILE_SIZE, null);
            gm.dispose();
            ...
            javax.swing.JDialog dlg = new javax.swing.JDialog(owner,
                    "Export Manager — " + tw + " × " + th + " tiles  @  (" + tileX + ", " + tileY + ")",
                    true);
            dlg.setLayout(new BorderLayout(8, 8));
            dlg.getContentPane().setBackground(new Color(25, 30, 45));

            // ── Panel de lista de tiles ──────────────────────────────────────
            JPanel tileListPanel = new JPanel();
            tileListPanel.setLayout(new BoxLayout(tileListPanel, BoxLayout.Y_AXIS));
            tileListPanel.setBackground(new Color(30, 35, 50));
            tileListPanel.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));

            JScrollPane tileScroll = new JScrollPane(tileListPanel);
            tileScroll.setPreferredSize(new Dimension(340, 420));
            javax.swing.border.TitledBorder tilesBorder = BorderFactory.createTitledBorder(
                    BorderFactory.createLineBorder(new Color(80, 100, 160)),
                    "Tiles únicas — ID fijo (guardadas) · ▲▼ solo nuevas");
            tilesBorder.setTitleColor(new Color(180, 200, 255));
            tilesBorder.setTitleFont(new Font(Font.SANS_SERIF, Font.BOLD, 12));
            tileScroll.setBorder(tilesBorder);
            tileScroll.getVerticalScrollBar().setUnitIncrement(16);

            // ── Rebuilder ─────────────────────────────────────────────────────
            final Runnable[] rebuilder = {null};
            rebuilder[0] = () -> {
                int scrollVal = tileScroll.getVerticalScrollBar().getValue();
                tileListPanel.removeAll();

                // Calcular IDs provisionales para tiles nuevas según su posición
                // entre las nuevas (no altera a las guardadas)
                int newCounter = nextNewId[0];
                java.util.IdentityHashMap<TileEntry, Integer> previewId =
                        new java.util.IdentityHashMap<>();
                for (TileEntry te : orderedTiles) {
                    if (te.name != null) {
                        // ID fijo de tile.info
                        previewId.put(te, nameToFixedId.getOrDefault(te.name.trim(), -1));
                    } else {
                        previewId.put(te, newCounter++);
                    }
                }

                for (int i = 0; i < orderedTiles.size(); i++) {
                    final int fi = i;
                    TileEntry te = orderedTiles.get(i);
                    boolean isSaved = (te.name != null);

                    JPanel rowPanel = new JPanel(new java.awt.FlowLayout(
                            java.awt.FlowLayout.LEFT, 5, 3));
                    rowPanel.setBackground(isSaved
                            ? new Color(25, 35, 25)   // fondo oscuro-verde para guardadas
                            : new Color(30, 35, 50));
                    rowPanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 74));

                    // ── Botones ▲ ▼ — solo activos para tiles nuevas ──────
                    javax.swing.JButton upBtn = new javax.swing.JButton("▲");
                    javax.swing.JButton dnBtn = new javax.swing.JButton("▼");
                    upBtn.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 10));
                    dnBtn.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 10));
                    upBtn.setMargin(new java.awt.Insets(1, 4, 1, 4));
                    dnBtn.setMargin(new java.awt.Insets(1, 4, 1, 4));
                    if (isSaved) {
                        // Tile guardada: ▲▼ desactivados, tooltip explicativo
                        upBtn.setEnabled(false);
                        dnBtn.setEnabled(false);
                        upBtn.setToolTipText("ID fijo — no se puede mover");
                        dnBtn.setToolTipText("ID fijo — no se puede mover");
                    } else {
                        // Tile nueva: solo puede moverse entre otras nuevas
                        // Buscar la tile nueva anterior y siguiente en orderedTiles
                        int prevNew = -1, nextNew = -1;
                        for (int k = fi - 1; k >= 0; k--)
                            if (orderedTiles.get(k).name == null) { prevNew = k; break; }
                        for (int k = fi + 1; k < orderedTiles.size(); k++)
                            if (orderedTiles.get(k).name == null) { nextNew = k; break; }
                        upBtn.setEnabled(prevNew >= 0);
                        dnBtn.setEnabled(nextNew >= 0);
                        if (prevNew >= 0) {
                            final int swapUp = prevNew;
                            upBtn.addActionListener(ev -> {
                                TileEntry tmp = orderedTiles.get(swapUp);
                                orderedTiles.set(swapUp, te);
                                orderedTiles.set(fi, tmp);
                                rebuilder[0].run();
                            });
                        }
                        if (nextNew >= 0) {
                            final int swapDn = nextNew;
                            dnBtn.addActionListener(ev -> {
                                TileEntry tmp = orderedTiles.get(swapDn);
                                orderedTiles.set(swapDn, te);
                                orderedTiles.set(fi, tmp);
                                rebuilder[0].run();
                            });
                        }
                    ...
                    // ── ID label ──────────────────────────────────────────
                    int dispId = previewId.get(te);
                    JLabel idLbl = new JLabel(dispId >= 0 ? String.valueOf(dispId) : "?");
                    idLbl.setForeground(isSaved
                            ? new Color(80, 220, 80)    // verde = fijo
                            : new Color(255, 200, 80)); // amarillo = provisional
                    idLbl.setFont(new Font(Font.MONOSPACED, Font.BOLD, 12));
                    idLbl.setPreferredSize(new Dimension(32, 20));
                    idLbl.setHorizontalAlignment(SwingConstants.RIGHT);
                    idLbl.setToolTipText(isSaved ? "ID fijo (tile.info)" : "ID provisional (siguiente disponible)");

                    // ── Preview ×4 ────────────────────────────────────────
                    JLabel prevLbl = new JLabel(new ImageIcon(scaleNearest(te.image, 4)));
                    prevLbl.setBorder(BorderFactory.createLineBorder(
                            isSaved ? new Color(40, 160, 40) : new Color(80, 100, 160), 1));

                    rowPanel.add(upBtn);
                    rowPanel.add(dnBtn);
                    rowPanel.add(idLbl);
                    rowPanel.add(prevLbl);

                    if (isSaved) {
                        JLabel nameLbl = new JLabel(te.name);
                        nameLbl.setForeground(new Color(100, 220, 100));
                        nameLbl.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
                        JLabel fixedLbl = new JLabel("🔒");
                        fixedLbl.setToolTipText("Tile guardada — ID inamovible");
                        rowPanel.add(nameLbl);
                        rowPanel.add(fixedLbl);
                    } else {
                        JTextField tf = nameFields.get(te);
                        JLabel warnLbl = new JLabel("⚠ sin nombre");
                        warnLbl.setForeground(new Color(255, 130, 60));
                        warnLbl.setFont(new Font(Font.SANS_SERIF, Font.ITALIC, 11));
                        rowPanel.add(tf);
                        rowPanel.add(warnLbl);
                    }

                    tileListPanel.add(rowPanel);
                    tileListPanel.add(javax.swing.Box.createVerticalStrut(2));
                }

                tileListPanel.revalidate();
                tileListPanel.repaint();
                javax.swing.SwingUtilities.invokeLater(() ->
                        tileScroll.getVerticalScrollBar().setValue(scrollVal));
            };
            rebuilder[0].run();

            // ── Preview del mapa ──────────────────────────────────────────────
            int dispScale = DISPLAY_SCALE * 2;
            int maxPx = 480;
            while (dispScale > 1 &&
                   (mapImage.getWidth() * dispScale > maxPx ||
                    mapImage.getHeight() * dispScale > maxPx))
                dispScale--;
            JLabel mapPreviewLbl = new JLabel(new ImageIcon(scaleNearest(mapImage, dispScale)));
            JScrollPane mapScroll = new JScrollPane(mapPreviewLbl);
            mapScroll.setPreferredSize(new Dimension(420, 420));
            javax.swing.border.TitledBorder mapBorder = BorderFactory.createTitledBorder(
                    BorderFactory.createLineBorder(new Color(60, 160, 80)),
                    "Mapa a exportar  (" + tw + " × " + th + " tiles)");
            mapBorder.setTitleColor(new Color(140, 255, 140));
            mapBorder.setTitleFont(new Font(Font.SANS_SERIF, Font.BOLD, 12));
            mapScroll.setBorder(mapBorder);
            mapScroll.getVerticalScrollBar().setUnitIncrement(16);
            mapScroll.getHorizontalScrollBar().setUnitIncrement(16);

            JSplitPane centerSplit = new JSplitPane(
                    JSplitPane.HORIZONTAL_SPLIT, tileScroll, mapScroll);
            centerSplit.setResizeWeight(0.40);
            centerSplit.setBackground(new Color(25, 30, 45));

            // ── Botones ───────────────────────────────────────────────────────
            JPanel btnPanel = new JPanel(new java.awt.FlowLayout(
                    java.awt.FlowLayout.RIGHT, 10, 8));
            btnPanel.setBackground(new Color(25, 30, 45));
            javax.swing.JButton btnAccept = new javax.swing.JButton("Aceptar");
            javax.swing.JButton btnCancel = new javax.swing.JButton("Cancelar");
            ...
            btnAccept.addActionListener(ev -> {
                // ── 1. Validar nombres ────────────────────────────────────
                java.util.List<String> nameErrors = new java.util.ArrayList<>();
                for (TileEntry te : orderedTiles)
                    if (te.name == null) {
                        JTextField tf = nameFields.get(te);
                        if (tf == null || tf.getText().trim().isEmpty())
                            nameErrors.add("  • Tile índice " + te.index + " sin nombre.");
                    }
                if (!nameErrors.isEmpty()) {
                    JOptionPane.showMessageDialog(dlg,
                            "No se puede exportar: tiles sin nombre:\n\n"
                            + String.join("\n", nameErrors),
                            "Nombres incompletos", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                // ── 2. Guardar tiles sin nombre ───────────────────────────
                boolean newTilesSaved = false;
                for (TileEntry te : orderedTiles)
                    if (te.name == null) {
                        String name = nameFields.get(te).getText().trim();
                        saveTileToFile(te.image, name);
                        te.name = name;  // actualizar en memoria
                        newTilesSaved = true;
                    }
                // Refrescar lista de tiles conocidas en la ventana principal
                if (newTilesSaved && onTilesUpdated != null)
                    javax.swing.SwingUtilities.invokeLater(onTilesUpdated);

                // ── 3. Seleccionar destino ZIP ────────────────────────────
                javax.swing.JFileChooser fc = new javax.swing.JFileChooser();
                fc.setDialogTitle("Guardar exportación ZIP");
                fc.setSelectedFile(new File(
                        String.format("map_%d_%d_%dx%d.zip", tileX, tileY, tw, th)));
                fc.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter(
                        "ZIP files (*.zip)", "zip"));
                if (fc.showSaveDialog(dlg) != javax.swing.JFileChooser.APPROVE_OPTION) return;
                File zipFile = fc.getSelectedFile();
                if (!zipFile.getName().toLowerCase().endsWith(".zip"))
                    zipFile = new File(zipFile.getAbsolutePath() + ".zip");

                // ── 4. Generar ZIP ────────────────────────────────────────
                // Los IDs provienen de tile.info (fuente de verdad).
                // Se lee DESPUÉS de haber guardado las tiles nuevas (paso 2),
                // para que sus entradas ya estén presentes en el fichero.
                java.util.Map<String, Integer> nameToId = new java.util.LinkedHashMap<>();
                File tileInfoFileExport = new File(TILE_INFO_SRC);
                if (tileInfoFileExport.exists()) {
                    try (java.io.BufferedReader br = new java.io.BufferedReader(
                            new java.io.FileReader(tileInfoFileExport, java.nio.charset.StandardCharsets.UTF_8))) {
                        String line;
                        boolean firstLine = true;
                        while ((line = br.readLine()) != null) {
                            if (firstLine) { firstLine = false; continue; } // cabecera
                            line = line.trim();
                            if (line.isEmpty()) continue;
                            String[] p = line.split(",", 3);
                            if (p.length >= 2) {
                                try { nameToId.put(p[1].trim(), Integer.parseInt(p[0].trim())); }
                                catch (NumberFormatException ignored) {}
                            }
                        }
                    } catch (IOException ex2) {
                        JOptionPane.showMessageDialog(dlg,
                                "No se pudo leer tile.info para resolver IDs:\n" + ex2.getMessage(),
                                "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }

                // Resolver ID para cada tile; si falta en tile.info (no debería) usar -1
                java.util.IdentityHashMap<TileEntry, Integer> tileId = new java.util.IdentityHashMap<>();
                for (TileEntry te : orderedTiles) {
                    Integer id = nameToId.get(te.name == null ? "" : te.name.trim());
                    tileId.put(te, id != null ? id : -1);
                }
                ...
                try (java.util.zip.ZipOutputStream zos = new java.util.zip.ZipOutputStream(
                    ...
                    // map.png
                    zos.putNextEntry(new java.util.zip.ZipEntry("map.png"));
                    ImageIO.write(mapImage, "PNG", zos);
                    zos.closeEntry();

                    // map.dat — cuadrícula de IDs leídos de tile.info
                    StringBuilder dat = new StringBuilder();
                    for (int r = 0; r < th; r++) {
                        for (int c = 0; c < tw; c++) {
                            if (c > 0) dat.append(' ');
                            dat.append(tileId.get(selGrid[r][c]));
                        }
                        dat.append('\n');
                    }
                    zos.putNextEntry(new java.util.zip.ZipEntry("map.dat"));
                    zos.write(dat.toString().getBytes(java.nio.charset.StandardCharsets.UTF_8));
                    zos.closeEntry();

                    // map.info — id,name,file  (IDs de tile.info, ordenado por ID ascendente)
                    java.util.List<TileEntry> sortedForInfo = new java.util.ArrayList<>(orderedTiles);
                    sortedForInfo.sort(java.util.Comparator.comparingInt(t -> tileId.get(t)));
                    StringBuilder info = new StringBuilder("id,name,file\n");
                    for (TileEntry te : sortedForInfo) {
                        String fname = te.name.trim()
                                .replaceAll("[^a-zA-Z0-9_\\-]", "_") + ".png";
                        info.append(tileId.get(te)).append(',')
                            .append(te.name).append(',')
                            .append("tiles/").append(fname).append('\n');
                    }
                    zos.putNextEntry(new java.util.zip.ZipEntry("map.info"));
                    ...
                    for (TileEntry te : orderedTiles) {
                        String fname = te.name.trim()
                                .replaceAll("[^a-zA-Z0-9_\\-]", "_") + ".png";
                        zos.putNextEntry(new java.util.zip.ZipEntry("tiles/" + fname));
                        ImageIO.write(te.image, "PNG", zos);
                        zos.closeEntry();
                    }
                ...
                JOptionPane.showMessageDialog(dlg,
                        ...
                        + "\n\nContenido del ZIP:"
                        + "\n  map.png   — imagen del mapa (" + tw + "×" + th + " tiles)"
                        + "\n  map.dat   — cuadrícula de IDs (posición en la lista)"
                        + "\n  map.info  — tabla  id / nombre / fichero"
                        + "\n  tiles/    — " + orderedTiles.size() + " tile(s) únicas",
            ...
            dlg.add(btnPanel, BorderLayout.SOUTH);
            dlg.setPreferredSize(new Dimension(760, 560));
            dlg.pack();
            dlg.setLocationRelativeTo(owner);
            dlg.setVisible(true);
            return exported[0];
