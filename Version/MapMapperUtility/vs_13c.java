    ...
    private static class SelectableMapPanel extends JPanel {
        ...
        private void installMouse() {
            java.awt.event.MouseAdapter ma = new java.awt.event.MouseAdapter() {
                ...
                public void mouseDragged(java.awt.event.MouseEvent e) {
                    ...
                    switch (dragMode) {
                        case NEW -> {
                            int x = Math.min(dragAnchor.x, p.x);
                            int y = Math.min(dragAnchor.y, p.y);
                            int w = Math.abs(p.x - dragAnchor.x);
                            int h = Math.abs(p.y - dragAnchor.y);
                            sel = new Rectangle(x, y, w, h);
                            repaint();
                        }
                        case MOVE -> {
                            int dx = p.x - dragAnchor.x;
                            int dy = p.y - dragAnchor.y;
                            sel = new Rectangle(selSnap.x + dx, selSnap.y + dy,
                                    selSnap.width, selSnap.height);
                            clampSel();
                            repaint();
                        }
                        case RESIZE -> {
                ...
                public void mouseReleased(java.awt.event.MouseEvent e) {
                    ...
                    // Descartar selección demasiado pequeña
        ...
        private void applyResize(Point p) {
            ...
            switch (resizeHandleIdx) {
                case 0 -> { x1 = p.x; y1 = p.y; }   // NW
                case 1 ->   y1 = p.y;                 // N
                case 2 -> { x2 = p.x; y1 = p.y; }    // NE
                case 3 ->   x2 = p.x;                 // E
                case 4 -> { x2 = p.x; y2 = p.y; }    // SE
                case 5 ->   y2 = p.y;                 // S
                case 6 -> { x1 = p.x; y2 = p.y; }    // SW
                case 7 ->   x1 = p.x;                 // W
            }
        ...
        protected void paintComponent(Graphics g) {
            ...
            int tileW = sel.width  / scale;
            int tileH = sel.height / scale;
            if (tileW < 1) tileW = 1;
            if (tileH < 1) tileH = 1;
