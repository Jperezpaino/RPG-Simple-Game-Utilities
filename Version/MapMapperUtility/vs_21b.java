...
public class MapMapperUtility {
    ...
    private static class SelectableMapPanel extends JPanel {
        ...
        /** Export Manager: lista reordenable de tiles únicas + preview del mapa.
         *  Botones ▲/▼ para reordenar; el índice en map.dat/map.info es la posición 0-based.
         *  Tiles sin nombre muestran un campo de texto obligatorio.
         *  Al aceptar genera un ZIP y devuelve true; actualiza la lista de tiles conocidas. */
        private static boolean showExportManagerDialog(JFrame owner, TileEntry[][] selGrid,
            ...
            // ── Lista ordenada de tiles únicas (mutable para reordenar) ──────
            ...
            for (TileEntry te : orderedTiles)
                if (te.name == null) {
                    ...
                    tf.setToolTipText("Nombre requerido para exportar (índice " + te.index + ")");
            ...
            tileScroll.setPreferredSize(new Dimension(320, 420));
            javax.swing.border.TitledBorder tilesBorder = BorderFactory.createTitledBorder(
                    ...
                    "Tiles únicas — ID = posición · ▲▼ reordenar");
            ...
            rebuilder[0] = () -> {
                ...
                for (int i = 0; i < orderedTiles.size(); i++) {
                    ...
                    rowPanel.setBackground(new Color(30, 35, 50));
                    ...
                    upBtn.setEnabled(fi > 0);
                    dnBtn.setEnabled(fi < orderedTiles.size() - 1);
                    upBtn.addActionListener(ev -> {
                        TileEntry tmp = orderedTiles.get(fi - 1);
                        orderedTiles.set(fi - 1, te);
                    ...
                    // ── ID (solo lectura, refleja la posición actual) ─────
                    JLabel idLbl = new JLabel(String.valueOf(i));
                    idLbl.setForeground(new Color(120, 180, 255));
                    idLbl.setFont(new Font(Font.MONOSPACED, Font.BOLD, 12));
                    idLbl.setPreferredSize(new Dimension(28, 20));
                    idLbl.setHorizontalAlignment(SwingConstants.RIGHT);
                    idLbl.setToolTipText("ID en map.dat / map.info");

                    // ── Preview ×4 ────────────────────────────────────────
                    JLabel prevLbl = new JLabel(new ImageIcon(scaleNearest(te.image, 4)));
                    prevLbl.setBorder(BorderFactory.createLineBorder(new Color(80, 100, 160), 1));
